...
function captcha($str){
    ...
    for($i=0;$i<mb_strlen($gstr);$i++){
        ...
        $tmp=imagettfbbox($fontsize,$text_info[$char]['angle'],realpath('./fonts/SHOWG.TTF'),$char);
    ...
    foreach($text_info as $char => $info){
        ...
        imagettftext($dst_img,$fontsize,$info['angle'],$x_pointer,$y,$colors[rand(0,count($colors)-1)],realpath('./fonts/SHOWG.TTF'),$char);
